...
def login():
	...
	us = inputD('[?] Email/HP')
	pa = inputD('[?] PASSWORD')
	cetak('!h[*] SEDANG LOGIN KE AKUN KAMU....')
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		cetak('!h[*] WELCOME TO MBF !k%s'%nama)
		cetak('!h[*] SEMOGA ANDA DAPAT KORBAN YANG BANYAK...')
		log = 1
	elif 'checkpoint' in url:
		cetak('!m[!] AKUN KAMU KENA CHECKPOINT\n!k[!]COBA LOGIN MENGGUNAKAN BROWSER')
		keluar()
	else:
		cetak('!m[!] LOGIN GAGAL/ERROR')
def idgroup():
	if log != 1:
		cetak('!h[*] LOGIN !bFACEBOOK!h DULU YA...')
		login()
		if log == 0:
			CLOSE()
	...
	while 1:
		...
		try:
			...
			break
	simpan()
	i = inputD('[?] LANGSUNG SAJA HACK (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
def idteman():
	if log != 1:
		cetak('!h[*] LOGIN !bFACEBOOK DULU !hdYA...')
		login()
		if log == 0:
			keluar()
